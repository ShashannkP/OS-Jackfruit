...
struct monitored_entry {
    ...
    int soft_warned;          /* 1 after the soft-limit warning has been emitted */
 ...
 * ============================================================== */
/*
 * We use a spinlock rather than a mutex because:
 *   - The timer callback runs in softirq context and cannot sleep,
 *     so a sleeping mutex is not usable there.
 *   - The ioctl paths are short (O(n) list scan at most), so the
 *     spin duration is bounded and low latency is desirable.
 */
